...
## Ways to Add Data
- Open Napkin Visual/demo. You should see the following prompt:

![Add data to the map pop up](./photos/image42.png "Add data to the map pop up")

**Napkin Visual is a pure client side app. Data lives only in your machine/browser.  No information or maps is sent back up to our server.**
...
| **From URL**  | Directly load data or map json by pasting a remote URL. You can link it to CSV | JSON | Napkin Visual config json. Make sure the url contains the file extension. CORS policy must be defined on your custom url domain. |
| **Sample data**  | Load one of Napkin Visual’s sample datasets. The sample map data and config are directly loaded from  [Napkin Visual-data github][Napkin Visual-data-github] repo  |


## Supported Projection Coordinate System
Napkin Visual only supports **[Web Mercator]([https://en.wikipedia.org/wiki/Web_Mercator_projection) EPSG:3857 -- WGS84**.
...
## Supported File Formats
 - [CSV](#csv)
 - [GeoJSON](#geojson)
 - [Napkin Visual Json](#Napkin-Visual-json): exported from Napkin Visual. See [export map](https://github.com/keplergl/kepler.gl/blob/master/docs/user-guides/k-save-and-export.md#export-map-as-json)


### CSV

CSV file should contain header row and multiple columns. Each row should be 1 feature. Each column should contain only 1 data type, based on which Napkin Visual will use to create layers and filters.
...
Because CSV file content is uploaded as strings, Napkin Visual will attempt to detect column data type by parsing a sample of data in each column. Napkin Visual can detect
...
**Note:** Make sure to clean up values such as `N/A`, `Null`, `\N`. If your column contains mixed type, Napkin Visual will treat it as **_`string`_** to be safe.

#### 2. Layer detection based on column names

Napkin Visual will auto detect layer, if the column names follows certain naming convention. Napkin Visual creates a point layer if  your CSV has columns that are named `<name>_lat` and `<name>_lng` or `<name>_latitude` and `<name>_longitude`, or `<name>_lat` and `<name>_lon`.
...
- Napkin Visual accepts GeoJSON formatted JSON that contains a single [Feature](https://tools.ietf.org/html/rfc7946#section-3.2) object or a [FeatureCollection](https://tools.ietf.org/html/rfc7946#section-3.3) object. Napkin Visual creates one **`Polygon`** layer per GeoJSON file.
  ...
  Napkin Visual will render all features in one `Polygon` layer even though they have different geometry types. Acceptable geometry types are
...
#### 2. Auto styling
Napkin Visual will read styles from GeoJSON files. If you are a GeoJSON expert, you can add style declarations to feature properties. Napkin Visual will use the declarations to automatically style your feature. The acceptable style properties are:
  ```json
  "properties": {
...
You load data or map through custom URL. It currently supports URLs with file extension of `csv`, `json` and `Napkin Visual.json`

In addition, this also by-passes 250mb file upload size limit which allows you to upload larger file to Napkin Visual.

![Load Map Using URL](./photos/a-load-map-using-url.gif "Load Map Using URL")

### Use Napkin Visual’s Sample Maps

The sample maps are a great option for new users to explore Napkin Visual and get a feel for how it works.

1. At the initial load prompt select “Try sample data” in the top right corner.

![Try sample data pop up](./photos/image2.png "Try sample data pop up")

2. Choose from the options to load the sample map and explore the configurations applied.

![Choose sample data pop up](./photos/image5.png "Choose sample data pop up")
...
![Add more data](./photos/image22.png "Add more data")

2. Choose one of the options above: upload a JSON/CSV file, or use Napkin Visual’s sample data.
